...
def kmeans_clustering(data):
    ...
    cluster_features = ['russia_GDP_Per_Capita', 'russia_CO2']
    ...
    for cluster in data['Cluster'].unique():
        ...
        plt.scatter(cluster_data['Year'], cluster_data['russia_CO2'], label=f'Cluster {cluster + 1}')

    plt.xlabel('Year')
    plt.ylabel('Russia CO2 Emissions')
    plt.title('Clustering of Russia CO2 Emissions over Time')
    plt.legend()
    plt.show()
    ...
    for cluster in data['Cluster'].unique():
        cluster_data = data[data['Cluster'] == cluster]
        plt.scatter(cluster_data['Year'], cluster_data['russia_GDP_Per_Capita'], label=f'Cluster {cluster + 1}')

    plt.xlabel('Year')
    plt.ylabel('Russia GDP_Per_Capita')
    plt.title('Clustering of Russia GDP_Per_Capita over Time')
    plt.legend()
    plt.show()
...
def curve_fitting(data):
    ...
    popt_gdp, pcov_gdp = curve_fit(linear_func, data['Year'], data['russia_GDP_Per_Capita'])
    future_years = np.arange(2021, 2031)
    predicted_values_gdp = linear_func(future_years, *popt_gdp)
    lower_bound_gdp, upper_bound_gdp = err_ranges(future_years, data['russia_GDP_Per_Capita'], linear_func, popt_gdp, pcov_gdp)

    # Curve fitting for CO2 Emissions
    popt_co2, pcov_co2 = curve_fit(linear_func, data['Year'], data['russia_CO2'])
    predicted_values_co2 = linear_func(future_years, *popt_co2)
    lower_bound_co2, upper_bound_co2 = err_ranges(future_years, data['russia_CO2'], linear_func, popt_co2, pcov_co2)

    # Plotting the curve fit with forecasting for GDP
    plt.figure(figsize=(12, 6))
    plt.scatter(data['Year'], data['russia_GDP_Per_Capita'], label='Actual Data (GDP)')
    ...
    plt.xlabel('Year')
    plt.ylabel('Russia GDP Per Capita')
    plt.title('Curve Fit and Forecast of Russia GDP Per Capita over Time')
    plt.legend()
    plt.show()

    # Plotting the curve fit with forecasting for CO2 Emissions
    plt.figure(figsize=(12, 6))
    plt.scatter(data['Year'], data['russia_CO2'], label='Actual Data (CO2)')
    ...
    plt.xlabel('Year')
    plt.ylabel('Russia CO2 Emissions')
    plt.title('Curve Fit and Forecast of Russia CO2 Emissions over Time')
    plt.legend()
    plt.show()
...
file_path = r"C:\Users\user\Desktop\attique\mam jasp again assignment\russiadata.csv"
